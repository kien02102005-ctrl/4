...
<body>
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 120px; text-align: center;">Tải Về</th>
    ...
    <script>
        ...
        function calculateKPITotals() {
            ...
            updateRankingTable(totalAdmin, totalSelf);
        }

        function updateRankingTable(adminScore, selfScore) {
            const finalScore = adminScore > 0 ? adminScore : selfScore;
            
            for (let i = 1; i <= 4; i++) {
                const checkEl = document.getElementById(`rank-check-${i}`);
                if (checkEl) checkEl.innerHTML = '';
            }

            const badgeEl = document.getElementById('kpi-ranking-result-display');
            let rankText = '';
            let rankClass = '';
            let targetCheckId = '';

            if (finalScore >= 90) {
                rankText = 'Hoàn thành xuất sắc nhiệm vụ';
                rankClass = 'rank-excel';
                targetCheckId = 'rank-check-1';
            } else if (finalScore >= 75) {
                rankText = 'Hoàn thành tốt nhiệm vụ';
                rankClass = 'rank-good';
                targetCheckId = 'rank-check-2';
            } else if (finalScore >= 50) {
                rankText = 'Hoàn thành nhiệm vụ';
                rankClass = 'rank-fair';
                targetCheckId = 'rank-check-3';
            } else {
                rankText = 'Không hoàn thành nhiệm vụ';
                rankClass = 'rank-poor';
                targetCheckId = 'rank-check-4';
            }

            const targetCheckEl = document.getElementById(targetCheckId);
            if (targetCheckEl) {
                targetCheckEl.innerHTML = '<i class="fa-solid fa-circle-check" style="color: #10b981; font-size: 18px;"></i>';
            }

            if (badgeEl) {
                badgeEl.innerText = rankText;
                badgeEl.className = `ranking-result-badge ${rankClass}`;
            }
        }
        ...
        async function exportKPIWord() {
            ...
            const targetUserInfo = registeredUsers.find(u => u.username === kpiTargetUser);
            const userKpiType = (targetUserInfo && targetUserInfo.kpiType) ? targetUserInfo.kpiType : 'staff';
            const roleName = userKpiType === 'leader' ? 'Lãnh đạo' : (userKpiType === 'cleaner' ? 'Lao công' : 'Cán bộ/Nhân viên');
            ...
            const monthFormatted = `THÁNG ${monthParts[1].padStart(2, '0')} NĂM ${monthParts[0]}`;

            // Border viền đen chuẩn bảng Word
            const standardCellBorder = {
                        ...
                        children: [
                            new TableCell({
                                ...
                                children: [
                                    ...
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [new TextRun({ text: "KHOA HÓA LÝ", bold: true, font: "Times New Roman", size: 22 })]
                                    })
                                ]
                            }),
                            new TableCell({
                                ...
                                children: [
                                    ...
                                    new Paragraph({
                                        alignment: AlignmentType.CENTER,
                                        children: [new TextRun({ text: "Độc lập - Tự do - Hạnh phúc", font: "Times New Roman", size: 22 })]
                                    })
                                ]
                            })
                        ]
            ...
            const tableHeaderRow = new TableRow({
                ...
                children: [
                    new TableCell({
                        ...
                        borders: standardCellBorder,
                        children: [new Paragraph({ children: [new TextRun({ text: "Nội Dung đánh giá", bold: true, font: "Times New Roman", size: 22 })] })]
                    }),
                    new TableCell({
            ...
            // 3. Xây dựng nội dung Bảng
            Object.keys(sectionMaxScores).forEach(secKey => {
                const secTitle = sectionTitles[secKey] || '';
                const maxSecScore = sectionMaxScores[secKey] || 0;

                // Mục Lớn A, B, C...
                kpiTableRows.push(new TableRow({
                    children: [
                        new TableCell({
                            columnSpan: 4,
                            borders: standardCellBorder,
                        ...
                        children: [
                            new TableCell({
                                ...
                                children: [
                                    new Paragraph({
                                        children: [
                                            new TextRun({ text: `${sub.code}. ${sub.title}`, bold: true, font: "Times New Roman", size: 22 })
                                        ]
                                    })
                                ]
                            })
                                ...
                                children: [
                                    new TableCell({
                                        ...
                                        children: [
                                            new Paragraph({
                                                children: [
                                                    new TextRun({ text: `${idx + 1}. ${item.title}`, font: "Times New Roman", size: 22 })
                                                ]
                                            })
                                        ]
                                    }),
                                    new TableCell({
                                        width: { size: 12, type: WidthType.PERCENTAGE },
                                        borders: standardCellBorder,
                                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: String(item.maxScore), font: "Times New Roman", size: 22 })] })]
                                    }),
                                    new TableCell({
                                        width: { size: 12, type: WidthType.PERCENTAGE },
                                        borders: standardCellBorder,
            ...
            kpiTableRows.push(new TableRow({
                children: [
                    new TableCell({
                        width: { size: 64, type: WidthType.PERCENTAGE },
                        borders: standardCellBorder,
                        children: [new Paragraph({ children: [new TextRun({ text: "Tổng điểm đánh giá:", bold: true, font: "Times New Roman", size: 22 })] })]
                    }),
                    new TableCell({
                        width: { size: 12, type: WidthType.PERCENTAGE },
                        borders: standardCellBorder,
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: totalMax.toFixed(1), bold: true, font: "Times New Roman", size: 22 })] })]
                    }),
                    new TableCell({
                        width: { size: 12, type: WidthType.PERCENTAGE },
                        borders: standardCellBorder,
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: totalSelf.toFixed(1), bold: true, font: "Times New Roman", size: 22 })] })]
                    }),
                    new TableCell({
                        width: { size: 12, type: WidthType.PERCENTAGE },
                        borders: standardCellBorder,
                        children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: totalAdmin.toFixed(1), bold: true, font: "Times New Roman", size: 22 })] })]
                    })
                ]
            }));

            // 5. Tính toán Tự động Kết quả Xếp loại cho File Word
            const finalScoreForWord = totalAdmin > 0 ? totalAdmin : totalSelf;
            let rankTitleForWord = '';
            let isExcel = false, isGood = false, isFair = false, isPoor = false;

            if (finalScoreForWord >= 90) {
                rankTitleForWord = "Hoàn thành xuất sắc nhiệm vụ";
                isExcel = true;
            } else if (finalScoreForWord >= 75) {
                rankTitleForWord = "Hoàn thành tốt nhiệm vụ";
                isGood = true;
            } else if (finalScoreForWord >= 50) {
                rankTitleForWord = "Hoàn thành nhiệm vụ";
                isFair = true;
            } else {
                rankTitleForWord = "Không hoàn thành nhiệm vụ";
                isPoor = true;
            }

            // 6. Tạo BẢNG XẾP LOẠI cho File Word
            const rankingTableForWord = new Table({
                width: { size: 100, type: WidthType.PERCENTAGE },
                rows: [
                    new TableRow({
                        children: [
                            new TableCell({
                                columnSpan: 3,
                                borders: standardCellBorder,
                                children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "BẢNG XẾP LOẠI", bold: true, font: "Times New Roman", size: 22 })] })]
                            })
                        ]
                    }),
                    new TableRow({
                        children: [
                            ...
                            new TableCell({
                                ...
                                borders: standardCellBorder,
                                children: [new Paragraph({ children: [new TextRun({ text: "Mức xếp loại", bold: true, font: "Times New Roman", size: 22 })] })]
                            }),
                            new TableCell({
                                width: { size: 25, type: WidthType.PERCENTAGE },
                                borders: standardCellBorder,
                                children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Mức điểm", bold: true, font: "Times New Roman", size: 22 })] })]
                            })
                        ]
                    }),
                    new TableRow({
                        children: [
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "1", font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ children: [new TextRun({ text: `Hoàn thành xuất sắc nhiệm vụ ${isExcel ? ' (✓)' : ''}`, bold: isExcel, font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Từ 90 điểm trở lên", font: "Times New Roman", size: 22 })] })] })
                        ]
                    }),
                    new TableRow({
                        children: [
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "2", font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ children: [new TextRun({ text: `Hoàn thành tốt nhiệm vụ ${isGood ? ' (✓)' : ''}`, bold: isGood, font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Từ 75 đến dưới 90 điểm", font: "Times New Roman", size: 22 })] })] })
                        ]
                    }),
                    new TableRow({
                        children: [
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "3", font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ children: [new TextRun({ text: `Hoàn thành nhiệm vụ ${isFair ? ' (✓)' : ''}`, bold: isFair, font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Từ 50 đến dưới 75 điểm", font: "Times New Roman", size: 22 })] })] })
                        ]
                    }),
                    new TableRow({
                        children: [
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "4", font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ children: [new TextRun({ text: `Không hoàn thành nhiệm vụ ${isPoor ? ' (✓)' : ''}`, bold: isPoor, font: "Times New Roman", size: 22 })] })] }),
                            new TableCell({ borders: standardCellBorder, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Dưới 50 điểm", font: "Times New Roman", size: 22 })] })] })
                        ]
                    }),
                    new TableRow({
                        children: [
            ...
            const doc = new Document({
                sections: [{
                    ...
                    children: [
                        ...
                        new Paragraph({
                            children: [
                                new TextRun({ text: `Họ và tên: ${kpiTargetUser}`, font: "Times New Roman", size: 22 }),
                            ]
                        }),
                        new Paragraph({
                            spacing: { after: 150 },
                            children: [
                                new TextRun({ text: `Chức vụ: ${roleName}`, font: "Times New Roman", size: 22 })
                            ]
                        }),
                        new Paragraph({
                            alignment: AlignmentType.CENTER,
                            ...
                            children: [
                                new TextRun({
                                    text: `PHIẾU ĐÁNH GIÁ KẾT QUẢ CÔNG VIỆC (${monthFormatted})`,
                                    bold: true,
                                    font: "Times New Roman",
                                    size: 24
                                })
